...
export class BindingBase {
    ...
    defaultListener(identifier, listener, observerCallbackHandler, observerParentCallbackHandler) {
        ...
        var observer = object._observe(observerCallbackHandler, identifier.propertyName); 
        listener.push(observer);
...
const core = {
    ...
    bindingsDisconnect: function ($e) {
        this.inverse($e, ($e, level) => {
            if (!$e[this.BINDINGS_PROPERTY_NAME]
                || $e[this.BINDINGS_PROPERTY_NAME].length == 0) {
                return;
            }

            if ($e.tagName && core.ignoreTagNames[$e.tagName] === true) {
                return;
            }
...
//library
export const $p = {
    bind: function (controller, $e, propArray = ['model']) {
        propArray?.forEach((value, index) => {
            if (controller[value]) {
                controller[value] = _proxify(controller[value], value);
            }
        });
        
        (Array.isArray($e) ? $e : [$e])
            .forEach($e => core.bindingsConnect(controller, $e));
    ...
    dependency: {
        singleton (instance) {
            dependencies[instance.constructor.name] = { instance };
        },

        transient (type, callback) {
            dependencies[type.name] = { callback };
        },

        resolve (type) {
            const dependencyDescriptor = dependencies[type.name];
            if (!dependencyDescriptor) {
                throw new Error('Dependency not registered -> ' + type.name);
            }

            if (dependencyDescriptor.callback) {
                return dependencyDescriptor.callback(type);
            }
            else {
    ...
    object: {
        copy: function (value, filter, refs) {
            return core.copy(value, filter, refs);
        }
    },
    ...
    db: {
        ...
        cursor: function (db, store, filterFn = null) {
            const promise = new Promise((resolve, reject) => {
                const transaction = db.transaction(store, 'readonly');
                const objectStore = transaction.objectStore(store);
                const result = [];
...
const _proxyHandlerSet = function (obj, prop, value, listener, set) {
    const isIgnored = prop.startsWith('_');
...
const _extendObserverMethods = function (obj, listener) {
    ...
    const _observe = function (callback, propertyName) {
        ...
        //new listener
        var proxyListener = new core.cstr.listener(callback, this, propertyName);
...
const _proxify = function (obj) {
    ...
    _extendObserverMethods(proxy, listener);
...
const _propertyListen = function (obj, propertyName) {
    ...
    _extendObserverMethods(obj, listener);
};
...
(function () {
    ...
    try { importCoreModule('./input-binding.js'); } catch { }
